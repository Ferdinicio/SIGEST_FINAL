  ...
  private SisAtendimento() {
   ...
   public List<Atendimento> getListAtendimento(){
      ...
      try {
		while(rs.next()) {
			  ...
			  a.setDescricao(rs.getString("descricao"));
                          ...
                          a.setTempoInicio(rs.getString("data"));
                          a.setHoraEncerramento(rs.getString("hora"));
	...
	try {
      ...
      PreparedStatement pstm = null;
	try {
		pstm = conn.prepareStatement("insert into public.atendimento (cliente,descricao,equipamento,observacao,status,data,hora) values (?,?,?,?,?,?,?) ");
		...
		pstm.setString(2, atendimento.getDescricao());
                ...
                pstm.setString(7, atendimento.getHoraInicio());
	...
	try {
      ...
      PreparedStatement pstm = null;
	try {
		pstm = conn.prepareStatement("update public.atendimento set observacao=?, status=?, data=?, hora=? where os=?");
		...
		pstm.setString(2, atendimento.getStatus());
                ...
                pstm.setString(4, atendimento.getHoraInicio());
                pstm.setInt(5, atendimento.getOs());
